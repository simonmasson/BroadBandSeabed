    ...
    switch (dgType)
        ...
        case 'XML0'
            ...
            for i=1:length(config)
                ...
                if sum((vec_freq==-1), 'omitnan')==1||any(freqs==vec_freq)
                    uuu=uuu+1;
            ...
            nb_samples=max(nb_samples_curr(idx_freq,:),nb_samples, 'omitnan');
            nb_pings=nb_pings+nb_pings_curr(idx_freq,:);
            nb_pings_asked=p.Results.PingRange(2)-p.Results.PingRange(1)+1;
            nb_pings=min(nb_pings,nb_pings_asked, 'omitnan');
...
for ii=1:length(filenames)
    ...
    if ii==1
        for i=1:length(data.config)
            nb_pings(i)=min(nb_pings(i),p.Results.PingRange(2)-p.Results.PingRange(1)+1, 'omitnan');
        ...
        switch (dgType)
            ...
            case 'XML0'
                ...
                elseif contains(t_line,'Parameter')
                    if sum(param_dg, 'omitnan')==length(CIDs_freq)
                        fread(fid, 1, 'int32', 'l');
                        continue
                    end
                    ...
                    if curr_ping(idx)>=p.Results.PingRange(1)&&curr_ping(idx)<=p.Results.PingRange(2)
                        ...
                        switch config(idx).TransceiverType
                            case list_WBTs()
                                ...
                                if (sampleCount > 0)
                                    temp = fread(fid,2*nSignals*sampleCount,'float32', 'l');
                                end

                                if (sampleCount > 0)
                                    data.pings(idx).comp_sig_1(1:data.pings(idx).sampleCount,curr_ping(idx)-p.Results.PingRange(1)+1)=temp(1:2*nSignals:end)+1i*temp(2:2*nSignals:end);
                                    if( nSignals > 1 )
                                        data.pings(idx).comp_sig_2(1:data.pings(idx).sampleCount,curr_ping(idx)-p.Results.PingRange(1)+1)=temp(3:2*nSignals:end)+1i*temp(4:2*nSignals:end);
                                        data.pings(idx).comp_sig_3(1:data.pings(idx).sampleCount,curr_ping(idx)-p.Results.PingRange(1)+1)=temp(5:2*nSignals:end)+1i*temp(6:2*nSignals:end);
                                        if( nSignals > 3 )
                                            data.pings(idx).comp_sig_4(1:data.pings(idx).sampleCount,curr_ping(idx)-p.Results.PingRange(1)+1)=temp(7:2*nSignals:end)+1i*temp(8:2*nSignals:end);
                                        end
                                    end
                                end
                                %!<DRP: 20211226
